...
var serviceSmartSaver=require('../services/db_services/smartAgentDbService');
...
function verifyTokens(req,res,next){
  ...
  router.get('/interestBalance',verifyTokens, function(req, res,next) {
   ...
   router.post('/sellTheTokensAgent',verifyTokens, function(req, res,next) {

    serviceSmartSaver.doSellThoseTokens(req.body).then( function(results) {
      res.setHeader('Content-Type', 'application/json');
      res.json(results);
    }
   ...
   router.post('/checkNumberExist',verifyTokens, function(req, res,next) {

    serviceSmartSaver.doesThisNumberExist(req.body).then( function(results) {
      res.setHeader('Content-Type', 'application/json');
      res.json(results);
    }
  ...
  router.post('/boxBalance',verifyTokens, function(req, res,next) {
...
router.post('/buySmartAgentTokens',verifyTokens,  function(req, res,next) {


  serviceSmartSaver .buySmartAgentTokensService( req.body)
  .then(function(results) {
    ...
    const sms = {
      ...
      message:
        ...
        "Check dashboard for receipt. Enjoy 10% return on sales!!"
    };
