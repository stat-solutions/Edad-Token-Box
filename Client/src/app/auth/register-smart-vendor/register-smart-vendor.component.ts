...
export class RegisterSmartVendorComponent implements OnInit {
  ...
  checkWhtherAgentIsRegistered(agentsNumber: string): any {
    ...
    return new Promise(resolve => {
      ...
      this.authService.isAgentRegistered(agentsNumber).subscribe(
        status => {
          resolve(status);
        });
 ...
 this.checkWhtherAgentIsRegistered(this.fval.agents_contact_number.value).then(results => {
  ...
  if (results) {
    ...
    this.spinner.hide();
    this.alertService.danger({
      html: '<b>' + 'The Agent\'s telephone number is not registered for SMART AGENCY' + '</b>' + '<br/>'
    });
